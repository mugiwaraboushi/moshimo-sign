...
#
# ============================================================================
# このファイルは arduino-esp32 (ESP32 Arduino Core) 同梱の espota.py のコピーです。
#
#   出所      : espressif/arduino-esp32 — hardware/esp32/3.3.11/tools/espota.py
#               https://github.com/espressif/arduino-esp32/blob/master/tools/espota.py
#   ライセンス: LGPL-2.1-or-later (arduino-esp32 本体と同じ)
#   取得元    : arduino-cli で導入した esp32:esp32@3.3.11 のコア同梱物
#   コピー日  : 2026-08-30
#
# 中身は無改変。この注記ブロックを足しただけです。
#
# なぜリポジトリに置くか:
#   もしものChromebookから実機へOTA書き込みするときに使う。ESP32コア一式
#   (約4GB) を入れなくても、git pull だけで書き込み道具が手に入るようにするため。
#
# 使い方 (同一LAN内から):
#   python3 tools/espota.py -i <実機のIP> -p 3232 -a <OTA_PASSWORD> \
#     -f <ビルド済み .bin> -r -d
#   ※ OTAが中断すると掲示板が消灯し、復旧にUSB接続が必要になります。
#     実施はファームウェア担当の承認のもとで行うこと (docs/firmware-build.md)。
# ============================================================================
#
